          ...
          <div className="px-1 py-1">
            <Menu.Item disabled>
              {({ active, disabled }) => (
                <button
                  className={`${
                    disabled ? 'text-gray-300' : active ? 'bg-violet-500 text-white' : 'text-gray-900'
                  } group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                >
                  {disabled ? (
                    <EditDisabledIcon className="mr-2 h-5 w-5" aria-hidden="true" />
                  ) : active ? (
...
function EditDisabledIcon(props) {
  return (
    <svg {...props} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M4 13V16H7L16 7L13 4L4 13Z" fill="#FFFFFF" stroke="#CCCCCC" strokeWidth="2" />
    </svg>
  );
}
